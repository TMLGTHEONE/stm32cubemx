...
{
	HAL_I2C_Mem_Write(&hi2c1, OLED_ADDR, 0x00, I2C_MEMADD_SIZE_8BIT, &cmd, 1, OLED_TIMEOUT);
}

void OLED_WriteData(uint8_t data)
{
	HAL_I2C_Mem_Write(&hi2c1, OLED_ADDR, 0x40, I2C_MEMADD_SIZE_8BIT, &data, 1, OLED_TIMEOUT);
}
...
void OLED_Init(void)
{
	...
	for (i = 0; i < 1000; i++) // 上电延时，确保STM32本身已经初始化完毕
	{
		for (j = 0; j < 1000; j++)
			;
	}
