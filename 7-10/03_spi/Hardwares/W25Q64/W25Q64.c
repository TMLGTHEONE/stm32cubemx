...
{
    return HAL_SPI_Transmit(&hspi, send_buf, size, 100);
}
...
{
    return HAL_SPI_Receive(&hspi, recv_buf, size, 100);
}
...
{
    return HAL_SPI_TransmitReceive(&hspi, send_buf, recv_buf, size, 100);
}
 ...
 */
HAL_StatusTypeDef W25QXX_ReadID(void)
{
    uint8_t send_data[4] = {ManufactDeviceID_CMD, 0x00, 0x00, 0x00}; // 待发送数据，命令+地址
    uint8_t recv_buf[3] = {0};                                       // recv_buf[0]存放Manufacture ID, recv_buf[1,2]存放Device ID
    ...
    if (HAL_OK != SPI_Transmit(send_data, 1))
    {
        HAL_GPIO_WritePin(W25Q64_CHIP_SELECT_GPIO_Port, W25Q64_CHIP_SELECT_Pin, GPIO_PIN_SET);
        return HAL_ERROR;
    }

    if (HAL_OK != SPI_Receive(recv_buf, 3))
    {
        HAL_GPIO_WritePin(W25Q64_CHIP_SELECT_GPIO_Port, W25Q64_CHIP_SELECT_Pin, GPIO_PIN_SET);
        return HAL_ERROR;
    }
    Manufacture_ID = recv_buf[0];
    Device_ID = recv_buf[1] << 8 | recv_buf[2];
...
{
    uint8_t cmd = Continuous_Read_Mode_Reset_CMD;
...
void W25QXX_Write(uint8_t *data, uint32_t addr, int nbytes)
{
    ...
    while (nbytes > 0)
    {
        uint8_t size = nbytes >= 256 ? 255 : nbytes;
        ...
        // 把buffer写入W25Q64
        W25QXX_Page_Program(buffer, addr + start, size);
